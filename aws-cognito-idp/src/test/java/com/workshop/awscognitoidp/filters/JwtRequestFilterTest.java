...
public class JwtRequestFilterTest {
    ...
    public void Should_Success_When_Filter_is_called() throws ServletException, IOException {
        ...
        JwtDataRetriever jwtDataRetriever = mock(JwtDataRetriever.class);
        when(jwtDataRetriever.getTokenRole("valid_token")).thenReturn("trainer");

        JwtRequestFilter jwtRequestFilter = new JwtRequestFilter();
        jwtRequestFilter.jwtValidator = jwtValidator;
        jwtRequestFilter.jwtDataRetriever = jwtDataRetriever;

        jwtRequestFilter.doFilterInternal(request, response, filterChain);

        verify(jwtValidator).validateJwtToken("valid_token");
        verify(jwtDataRetriever).getTokenRole("valid_token");
